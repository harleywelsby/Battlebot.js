        ...
        if(splitargs[1].toLowerCase() === 'mine'){
            if(!players.has(msg.author.id)){
                ...
                for(let i=0; i<players.get(msg.author.id).movelist.length; i++){
                    ...
                    var newmove = {
                        name: move.name,
                        level: players.get(msg.author.id).movelist[i].split(' ')[0],
                        hp: move.hp,
                        type: move.type
                    }
                    toSend += 'L.' + players.get(msg.author.id).movelist[i] + ` (${getMoveDamage(newmove)}dmg, ${getMoveAccuracy(newmove)}acc)\n`;
                }
                msg.reply(toSend);
            }
        }
